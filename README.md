...
A production-grade Android AI assistant built with **Kotlin**, **Jetpack Compose (Material 3)**, **MVVM + Clean Architecture**, **Hilt**, **Coroutines/Flow**, **Room**, **DataStore**, **WorkManager**, and the **NVIDIA NIM** inference API. No accounts, no sign-in — open the app and start chatting.
...
| **Screens** | Home, Chat, Explore (prompt catalog), Library (grid/list + filters), Settings, Global Search, Voice mode |
...
| **Platform** | WorkManager daily reminders, connectivity-aware offline UI |
...
> The app is **100% local** — no account or sign-in required.
> AI responses require a valid `NVIDIA_API_KEY`; without it the app still builds and runs,
> the AI just can't answer.
...
- HTTPS-only; NVIDIA API key added via `AuthInterceptor` (network layer, not user auth).
- All chats live on-device in Room — nothing is synced to any server except the AI API call itself.
